...
public class Database {
    ...
    Database(String databasePath){
        ...
        try (Connection connection = DriverManager.getConnection(databasePath);
            ...
            statement.execute("CREATE TABLE IF NOT EXISTS movies (name TEXT, stars INTEGER, watched INTEGER )");
    ...
    public List<Movie> getAllMovies (){
        ...
        Statement statement = connection.createStatement()){
            ...
            while (movieResults.next()){
                ...
                boolean watched = movieResults.getBoolean("stars");
    ...
    public List<Movie> getAllMoviesByWatched(boolean watchedStatus){
        ...
        PreparedStatement preparedStatement = connection.prepareStatement("SELECT * FROM movies WHERE watched = ?")){
            ...
            ResultSet movieResuluts = preparedStatement.executeQuery();

            List<Movie> movies =new ArrayList<>();

            while (movieResuluts.next()) {
                String name = movieResuluts.getString("name");
                int stars = movieResuluts.getInt("stars");
                boolean watched = movieResuluts.getBoolean("watched");
                Movie movie = new Movie(name,stars,watched);
                movies.add(movie);
            }
